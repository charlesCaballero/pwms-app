...
export default function UpdateDisposalRequest(){
    ...
    const updateDisposalRequest = useMutation((data: any) => {
        return api(Method.PUT, `${inventoryMutation}/${'disposal'}/${selected['id']}`, data) as AxiosPromise<any>;
    });
    ...
    const handleSaveRequest = async() => {
      selected['disposal']['remarks'] = remarks;
        await updateDisposalRequest.mutate(selected, {
        ...
        {/* <ActionAlert show={showAlert} close={handleCloseAlert} action={action} /> */ }
            ...
            <Grid item>
                <Box sx={ { display: 'flex', m: '20px' } } justifyContent = "center">
                    <DataTable
                        header={ tableHeader }
                        dataQuery = { url + "/" + inventoryQuery +'/disposal'
                        }
            ...
            <ConfirmRequestDialog
                ...
                request={"close"}
                action={"save"}
                isLoading={updateDisposalRequest.isLoading}
            />
